...
const IngressPage = () => {
  ...
  const { wsStatus, data } = useWebSocketResourceList(
    `${API_WS_URL}/api/k8s/ws/resource-watcher/list/Ingress/${clientCurrentNamepace}`,
    clientCurrentNamepace,
    showAlertNotificationDisplay,
    setClientCurrentNamepaceError
  );
        ...
        <div className="items-center justify-between lg:flex">
          <div className="mb-4 lg:mb-0 flex">
            ...
            <h3 className="mb-2 text-xl font-bold text-gray-900 dark:text-white">
              Ingresses for Namespace: {clientCurrentNamepace}
            </h3>
          </div>
                      ...
                      filteredData.map((item) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            {item?.loadBalancers ? (
                              <div>
                                {item.loadBalancers.map((ip, index) => (
                          ...
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap truncate xl:max-w-xs dark:text-gray-400">
                            {item?.rules ? (
                              <div>
                                {item.rules.map((rule, index) => (
                                  <div key={index} className="mb-4">
                                    {rule.http &&
                                    rule.http.paths &&
                                    rule.http.paths.length > 0 ? (
                                      rule.http.paths.map((path, pathIndex) => (
                                        <div key={pathIndex} className="mb-2">
                                          <a
                                            href={`http://${rule.host}${path.path}`}
                                            target="_blank"
                                            className="block text-primary-600 dark:text-primary-500 hover:underline"
                                          >
                                            {`http://${rule.host}${path.path} ⇢ ${path.backend.service.name}:${path.backend.service.port.number}`}
                                          </a>
                                        </div>
                                      ))
                                    ) : (
