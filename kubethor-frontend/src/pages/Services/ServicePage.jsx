...
const ServicePage = () => {
  ...
  const { wsStatus, data } = useWebSocketResourceList(
    `${API_WS_URL}/api/k8s/ws/resource-watcher/list/Service/${clientCurrentNamepace}`,
    clientCurrentNamepace,
    showAlertNotificationDisplay,
    setClientCurrentNamepaceError
  );
        ...
        <div className="items-center justify-between lg:flex">
          <div className="mb-4 lg:mb-0 flex">
            ...
            <h3 className="mb-2 text-xl font-bold text-gray-900 dark:text-white">
              Service for Namespace: {clientCurrentNamepace}
            </h3>
          </div>
                    ...
                    {filteredData.length === 0 ? (
                      ...
                      filteredData.map((item) => (
                        ...
                        >
                            ...
                            {item.ports && item.ports.length > 0 ? (
                              item.ports.map((portObj, index) => (
                                ...
                                >
                                  ...
                                  {portObj.port}:{portObj.targetPort}/
                                  {portObj.protocol}
                                </span>
                              ))
                            ) : (
                          ...
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            {item.externalIPs && item.externalIPs.length > 0 ? (
                              item.externalIPs.map((externalIP, index) => (
                                <span
                                  key={index}
                                  className="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300"
                                >
                              ...
                              <p>-</p> // Optional: display a message if there are no external IPs
                            )}
                          </td>
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
