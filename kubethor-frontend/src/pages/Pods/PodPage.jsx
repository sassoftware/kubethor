...
const PodPage = () => {
  ...
  const { wsStatus, data } = useWebSocketResourceList(
    `${API_WS_URL}/api/k8s/ws/resource-watcher/list/Pod/${clientCurrentNamepace}`,
    clientCurrentNamepace,
    showAlertNotificationDisplay,
    setClientCurrentNamepaceError
  );
  ...
  const [selectedPodContainerName, setSelectedPodContainerName] =
    useState(null);
  const [isPodLogOpen, setIsPodLogOpen] = useState(false);
  const openPodLogDrawer = (containerName, podName) => {
    setSelectedPodName(podName);
    setSelectedPodContainerName(containerName);
    setIsPodLogOpen(true);
    setIsResourceEditorOpen(false);
  };
  const closePodLogDrawer = () => {
    setIsPodLogOpen(false);
  };

  // Pod Container Icons
  const getContainerDetails = (containerData, podName) => {
    const icons = [];
    for (const key in containerData) {
      const container = containerData[key];
      // console.log(containerData);
      if (container.containerState) {
        let title = `Container: ${container.containerName}, Status: ${container.containerState}`;

        if (
          container.containerState === "waiting" ||
          container.containerState === "terminated"
        ) {
          title += `, Reason: ${container.containerStateReason}`;
        }

        if (container.containerStartedAt) {
          title += `, StartedAt: ${container.containerStartedAt}`;
        }

        icons.push(
          <div
            key={container.containerName}
            className={`h-2.5 w-2.5 rounded mr-2 ${
              container.containerState === "running"
                ? "bg-green-400"
                : container.containerState === "pending"
                ? "bg-yellow-400"
                : container.containerState === "waiting"
                ? "bg-orange-400"
                : container.containerState === "terminated"
            ...
            title={title}
            onClick={() => openPodLogDrawer(container.containerName, podName)}
          ></div>
        );
      }
    }
  ...
  // Pod Containers Status Icon
  const getContainerStatus = (containerData) => {
    for (const key in containerData) {
      const container = containerData[key];
      if (container.containerState) {
        if (container.containerState === "waiting") {
          return (
            <svg
              ...
              height="16"
              fill="currentColor"
              className="flex-shrink-0 inline w-4 h-4 ml-2 mb-1 text-yellow-300"
              viewBox="0 0 16 16"
            >
  ...
  // Pod Status Icon
  const getStatusIcon = (status, statusReason) => {
    switch (status) {
      case "Running":
        return (
          <span className="bg-purple-100 text-purple-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md border border-purple-100 dark:bg-gray-700 dark:border-purple-500 dark:text-purple-400">
            Running{" "}
            {statusReason && <span className="text-xs"> - {statusReason}</span>}
          </span>
        );
      case "Pending":
        return (
      ...
      case "Failed":
        return (
          <span className="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md border border-red-100 dark:border-red-400 dark:bg-gray-700 dark:text-red-400">
            Failed{" "}
            {statusReason && (
              <span className="text-xs text-yellow-300"> - {statusReason}</span>
            )}
          </span>
        );
      default:
        return null; // If the status is not recognized, return null or a default icon
    }
  };
  ...
  return (
    <>
        ...
        <div className="items-center justify-between lg:flex">
          <div className="mb-4 lg:mb-0 flex">
            ...
            <h3 className="mb-2 text-xl font-bold text-gray-900 dark:text-white">
              Pods for Namespace: {clientCurrentNamepace}
            </h3>
          </div>
                    ...
                    {filteredData.length === 0 ? (
                      ...
                      filteredData.map((item) => (
                        ...
                        >
                          ...
                          >
                            {item.name} {getContainerStatus(item.containers)}
                          </td>
                          ...
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            <div className="flex items-center">
                              {getContainerDetails(item.containers, item.name)}{" "}
                              {getContainerDetails(
                                item.initContainers,
                                item.name
                              )}
                          ...
                          <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            {getStatusIcon(item.status, item.statusReason)}
                          </td>
      ...
      {isPodLogOpen && (
        <PodLog
          isOpen={isPodLogOpen}
          onClose={closePodLogDrawer}
          containerName={selectedPodContainerName}
