...
const EndpointDetail = ({ isOpen, onClose, resourceName }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        {resourceDetail?.subsets && resourceDetail.subsets.length > 0 ? (
          <div>
            ...
            <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {resourceDetail.subsets.map((subset, subsetIndex) => (
                  <React.Fragment key={subsetIndex}>
                    {subset.addresses && subset.addresses.length > 0 ? (
                      subset.addresses.map((address, addrIndex) => (
                        ...
                        >
                          <td className="px-6 py-4">{address.ip}</td>
                          <td className="px-6 py-4">
                            {address.hostname || "-"}
                          </td>
                          <td className="px-6 py-4">
            ...
            <h3 className="text-base font-semibold mt-8 mb-2">Ports</h3>
            <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  <th scope="col" className="px-6 py-3">
                  ...
                  <React.Fragment key={subsetIndex}>
                    {subset.ports && subset.ports.length > 0 ? (
                      subset.ports.map((port, portIndex) => (
                        <tr
                          key={`${subsetIndex}-${portIndex}`}
                          className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                        >
                          <td className="px-6 py-4">{port.port}</td>
                          <td className="px-6 py-4">{port.name || "-"}</td>
